  ...
  k6 run -o influxdb=http://1.2.3.4:8086/k6`[1:],
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !runtimeOptions.NoSummary.Bool {
				go func() {
					err := readInput(initRunner, globalCtx, engine, executionState, logger)
					if err != nil {
						logger.Error(err)
					}
				}()
			...
			if !runtimeOptions.NoSummary.Bool {
				printSummaryResults(initRunner, globalCtx, engine, executionState, logger)
			}
...
func printSummaryResults(runner lib.Runner, globalCtx context.Context, engine *core.Engine, executionState *lib.ExecutionState, log *logrus.Logger) {
...
func readInput(runner lib.Runner, globalCtx context.Context, engine *core.Engine, executionState *lib.ExecutionState, log *logrus.Logger) error {
...
forLoop:
	for {
		...
		case event := <-keysEvents:
			...
			if event.Rune == 'R' {
				printSummaryResults(runner, globalCtx, engine, executionState, log)
			}
